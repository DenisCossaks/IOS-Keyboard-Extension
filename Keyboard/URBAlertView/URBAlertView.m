...
- (void)animateWithType:(URBAlertAnimation)animation show:(BOOL)show completionBlock:(void (^)())completion {
    CGSize screenSize = self.parent.frame.size; ////[UIScreen mainScreen].bounds.size;
...
//	UIWindow *window = [[UIWindow alloc] initWithFrame:[[UIScreen mainScreen] bounds]];
    CGRect windowFrame = self.parent.frame;
	CGRect dialogFrame = self.frame;
	dialogFrame.origin.x = (CGRectGetWidth(windowFrame) - CGRectGetWidth(dialogFrame)) / 2.0;
	dialogFrame.origin.y = (CGRectGetHeight(windowFrame) - CGRectGetHeight(dialogFrame)) / 2.0;
...
- (CGRect)defaultFrame {
    CGRect appFrame = CGRectMake(0, 0, 320, 216); //[[UIScreen mainScreen] applicationFrame];
...
//			overlay.frame = self.window.bounds;
            overlay.frame = self.parent.bounds;
			overlay.alpha = 0.0;
		}
...
- (void)reposition {
    ...
    CGFloat duration = 0.4; //[UIApplication sharedApplication].statusBarOrientationAnimationDuration;
